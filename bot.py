...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        # Send the embed and delete it after 30 seconds
...
async def check_status():
    ...
    for guild_id, user_ids in server_user_ids.items():
        ...
        if not guild:
            print(f"Guild with ID {guild_id} not found.")  # Debug log
            continue
...
async def on_guild_join(guild):
    ...
    # Extract the "About" and "Commands" sections from the README
    about_section = []
    commands_section = []
    in_about = False
    in_commands = False
...
async def track_command(ctx, *inputs: str):
    guild_id = str(ctx.guild.id)  # Get the server (guild) ID as a string
    if guild_id not in server_user_ids:
        server_user_ids[guild_id] = []  # Initialize the list for this server

    # Get the Added/Removed Users Logs channel
    guild = ctx.guild
    soul_category = discord.utils.get(guild.categories, name="Soul")
    ...
    for input_value in inputs:
        ...
        if input_value.isdigit():
            ...
            try:
                ...
                data = response.json()
                if not data["data"]:
                    # Send an embed for invalid input
        ...
        if user_id in server_user_ids[guild_id]:
            # Fetch user details for the embed
            user_details = get_user_details(user_id)
            ...
            # Send an embed for already tracked users
            ...
            # Log the added user to the logs channel
            if logs_channel:
...
async def untrack_command(ctx, user_input: str):
    ...
    if user_id in server_user_ids[guild_id]:
        ...
        # Log the removed user to the logs channel
        if logs_channel:
            log_embed = discord.Embed(
...
async def whois_display_command(ctx, *, display_name: str):
    ...
    # Paginate results if there are multiple users
    pages = []
    for user in users:
    ...
    view = View(timeout=30)  # Auto-delete after 30 seconds of inactivity
    if len(pages) > 1:
...
async def changelog_command(ctx, *, message: str):
    ...
    # Call the send_changelog_update function
    await send_changelog_update(guild, message)

    # Confirm the changelog was sent
...
async def send_changelog_update(guild, message: str):
    ...
    """
    # Get the Soul category and Changelogs channel
    soul_category = discord.utils.get(guild.categories, name="Soul")
...
async def send_changelog_to_all_guilds(changelog_message: str, version: str):
    ...
    # Save the latest changelog version
